...
static const DebugMonValueEntry sDebugMonValueList[MAX_DEBUG_MON_STAT] = {
    ...
    [DEBUG_MON_STATUS] = { dmm_label_status, &sDebugValueStatus },
    ...
    [DEBUG_MON_EGG_LOCATION] = { dmm_label_egg_location, &sDebugValueLocation },
    ...
    [DEBUG_MON_EGG_YEAR] = { dmm_label_year, &sDebugValueYear },
    [DEBUG_MON_EGG_MONTH] = { dmm_label_month, &sDebugValueMonth },
    [DEBUG_MON_EGG_DAY] = { dmm_label_day, &sDebugValueDay },
...
static u8 DebugMonMenu_AddMon(DebugMonMenu *monMenu)
{
    ...
    if (monMenu->mode == DEBUG_MON_MENU_MODE_CREATE) {
        ...
        PCBoxes_TryStoreBoxMon(SaveData_GetPCBoxes(monMenu->fieldSystem->saveData), Pokemon_GetBoxPokemon(monMenu->mon.monData));
        ...
        if (monMenu->mon.stats[DEBUG_MON_IS_EGG]) {
            buf = MessageLoader_GetNewString(monMenu->msgLoader, 112);
...
static void DebugMon_InitStats(DebugMon *mon)
{
    ...
    mon->stats[DEBUG_MON_EGG_YEAR] = 6;
    mon->stats[DEBUG_MON_EGG_MONTH] = 9;
    mon->stats[DEBUG_MON_EGG_DAY] = 28;
...
static void DebugMon_CalcFullStats(DebugMonMenu *monMenu, DebugMon *mon)
{
    ...
    Pokemon_SetValue(mon->monData, MON_DATA_STATUS, &sStatusConditionTable[mon->stats[DEBUG_MON_STATUS]][1]);
    ...
    Pokemon_SetValue(mon->monData, MON_DATA_EGG_LOCATION, &mon->stats[DEBUG_MON_EGG_LOCATION]);
    Pokemon_SetValue(mon->monData, MON_DATA_EGG_YEAR, &mon->stats[DEBUG_MON_EGG_YEAR]);
    Pokemon_SetValue(mon->monData, MON_DATA_EGG_MONTH, &mon->stats[DEBUG_MON_EGG_MONTH]);
    Pokemon_SetValue(mon->monData, MON_DATA_EGG_DAY, &mon->stats[DEBUG_MON_EGG_DAY]);
    ...
    if (mon->stats[DEBUG_MON_IS_EGG]) {
        buf = MessageLoader_GetNewString(monMenu->msgLoader, dmm_debug_egg_name);
        Pokemon_SetValue(mon->monData, MON_DATA_NICKNAME_STRING, buf);
        String_Free(buf);
    }

    buf = MessageLoader_GetNewString(monMenu->msgLoader, dmm_debug_ot);
...
static void DebugMon_SetStatsFromMonData(DebugMon *mon)
{
    ...
    mon->stats[DEBUG_MON_EGG_LOCATION] = Pokemon_GetValue(mon->monData, MON_DATA_EGG_LOCATION, NULL);
    mon->stats[DEBUG_MON_EGG_YEAR] = Pokemon_GetValue(mon->monData, MON_DATA_EGG_YEAR, NULL);
    mon->stats[DEBUG_MON_EGG_MONTH] = Pokemon_GetValue(mon->monData, MON_DATA_EGG_MONTH, NULL);
    mon->stats[DEBUG_MON_EGG_DAY] = Pokemon_GetValue(mon->monData, MON_DATA_EGG_DAY, NULL);
...
static void DebugMonValue_PrintNum(Window *window, MessageLoader *msgLoader, StringTemplate *strTemplate, DebugMon *mon, u32 num, u32 digits, u32 x, u32 y, u32 delay, u32 color)
{
    String *buf = MessageLoader_GetNewString(msgLoader, dmm_template_number);

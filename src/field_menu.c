...
BOOL sub_0203B7C0(FieldTask *taskMan)
{
    ...
    case 1:
        PokemonSummary *v3 = Heap_AllocFromHeap(HEAP_ID_FIELDMAP, sizeof(PokemonSummary));

        v3->monData = Party_GetFromSavedata(fieldSystem->saveData);
        v3->options = SaveData_Options(fieldSystem->saveData);
        v3->dataType = SUMMARY_DATA_PARTY_MON;
        v3->monIndex = partyMan->unk_22;
        v3->monMax = Party_GetCurrentCount(v3->monData);
        v3->move = 0;
        v3->mode = SUMMARY_MODE_NORMAL;
